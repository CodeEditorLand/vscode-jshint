...
export const libraryConfirmationType = new RequestType<
	LibraryUsageConfirmationParams,
	boolean,
	void,
	void
...
export const errorNotificationType = new RequestType<
	ErrorNotificationParams,
	void,
	void,
	void
>("jshint/errorNotification");
...
class OptionsResolver {
	...
	private readOptions(fsPath: string = null): any {
		...
		function readJsonFile(file: string, extendedFrom?: string): any {
			try {
				return JSON.parse(
					stripComments(fs.readFileSync(file).toString())
				);
			} catch (err) {
		...
		function readJSHintFile(file: string, extendedFrom?: string): any {
			...
			if (content.extends) {
				...
				if (fs.existsSync(baseFile)) {
					content = _.mergeWith(
						readJSHintFile(baseFile, file),
						content,
						(baseValue, contentValue) => {
					...
					that.connection.window.showErrorMessage(
						`Can't find JSHint file ${baseFile} extended from ${file}`
					);
				}
			...
			if (content.overrides) {
				Object.keys(content.overrides).forEach((pathPattern) => {
					if (minimatch(fsPath, pathPattern)) {
						const optionsToOverride =
							content.overrides[pathPattern];

						if (optionsToOverride.globals) {
							content.globals = _.extend(
								content.globals || {},
		...
		if (
			jshintOptions &&
			jshintOptions.config &&
			fs.existsSync(jshintOptions.config)
		) {
		...
		if (fsPath) {
			...
			if (this.configPath && fs.existsSync(this.configPath)) {
				shouldBeExcluded = this.match(
					processIgnoreFile(this.configPath, [], { cache: false }),
					fsPath,
					root
				);
			} else {
...
class Linter {
	...
	async getLib(): Promise<any> {
		...
		const globalPath = this.getGlobalPackageManagerPath(
	...
	private getSeverity(problem: JSHintError): DiagnosticSeverity {
		...
		if (
			!problem.code ||
			problem.code[0] === "E" ||
			this.settings.reportWarningsAsErrors
		) {
	...
	private getMessage(err: any, document: TextDocument): string {
		...
		if (typeof err.message === "string" || err.message instanceof String) {
			...
			result = `An unknown error occured while validating file: ${Files.uriToFilePath(
				document.uri
			)}`;
		}
		return result;
