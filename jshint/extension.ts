...
export async function activate(context: ExtensionContext) {
	...
	let clientOptions: LanguageClientOptions = {
		...
		synchronize: {
			...
			fileEvents: [
				workspace.createFileSystemWatcher("**/.jshint{rc,ignore}"),
	...
	context.subscriptions.push(
		commands.registerCommand("jshint.resetLibraryExecutions", async () => {
			...
			const selected = await window.showQuickPick<JSHintQuickPickItem>(
				items,
	...
	client.onRequest(libraryConfirmationType, async (params) => {
		...
		const item = await window.showInformationMessage(
			...
			{ modal: true },
			{
				...
				value: ConfirmationSelection.alwaysAllow,
			},
